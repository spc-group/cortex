...
// export const useArrayStatsZ = (path: string, rois, root) => {
//   const zArr = useArrayZ(path, root);
//   const [stats, setStats] = useState([]);
//   // Clear the arrays for a new path
//   useEffect(() => {
//     setStats([]);
//   }, [path]);
//   // Set up new empty arrays for each ROI if they have changed
//   const roiString = JSON.stringify(
//     rois.map((r) => [r.x0, r.x1, r.y0, r.y1]).flat(),
//   );
//   useEffect(() => {
//     let hasChanged = false;
//     const newStats = rois.map((roi, index) => {
//       if (stats?.[index] != null && compareROIs(stats[index], roi)) {
//         return stats[index];
//       } else {
//         hasChanged = true;
//         return { sum: [], max: [], min: [], shape: [], roi: roi };
//       }
//     });
//     if (hasChanged) {
//       setStats(newStats);
...
// Hook that provides array data from a Zarr array
// export const useArrayZ = (
//   path: string,
//   slice: number | ndzarr.slice | null = null,
// ) => {
//   const zArr = useZArray(path);
//   const [arr, setArr] = useState(null);
//   useEffect(() => {
//     const getArray = async () => {
//       console.log(Object.keys(zArr));
//       console.log(zArr);
//       zArr.shape = [200, ...zArr.shape.slice(1, null)]
//       const newArr = await ndzarr.get(zArr, [200, null, null]);
//       setArr(newArr);
//     };
//     if (zArr != null) {
//       getArray().catch(console.error);
...
//           // const supportedMethods = ['GET']; // defaults
//           // const store = new HTTPStore('http://localhost:8000/zarr/v2', { fetchOptions, supportedMethods });
...
//           // });
//           // console.log(zRoot);
//           // const zArray = await zarr.open.v3(zRoot.resolve(path));
//           // const t0 = performance.now();
//           // const arr = await ndzarr.get(zArray);
//           // console.log(sum)
//           // sum(arr);
//           // console.log(performance.now() - t0);
//           // const zarrArray = await getArraySliceZarr(z, slice);
